...
public class GameController {
    ...
    // --- Support pour les deux modes d'affichage : avatars dynamiques OU avatar1/avatar2 + thème ---
    // Pour main branch (avatars dynamiques)
    ...
    // Pour branche thème/avatar fixe
    private Theme theme;
    private Image avatarP1, avatarP2;
    ...
    // --- Unifie le démarrage (main branch ou thème branch) ---
    public void startGame() {
        // On choisit le mode selon si theme est renseigné ou non
        if (theme == null) {
            // --- MODE MAIN BRANCH ---
    ...
    // --- Pour la branche thème : resize window ---
    private void resizeWindowToFitGame(double canvasWidth, double canvasHeight) {
        if (stage != null) {
            double windowDecorationWidth = 16;
            double windowDecorationHeight = 40;
    ...
    private void returnToMenu() {
        ...
        try {
            ...
            if (theme != null && stage != null) {
                // Remettre la fenêtre redimensionnable avant de retourner au menu (thème branch)
    ...
    private void handleKeyPressed(KeyEvent event) {
        ...
        // --- main branch key handling (humains/IA) ---
        if (theme == null) {
    ...
    private void drawGrid() {
        ...
        if (theme == null) {
            canvasWidth = borderPixel * 2 + gridWidth * CELL_SIZE;
            canvasHeight = topUiHeight + borderPixel * 2 + gridHeight * CELL_SIZE;
        } else {
            canvasWidth = borderPixel + gridWidth * CELL_SIZE + borderPixel;
            canvasHeight = topUiHeight + borderPixel + gridHeight * CELL_SIZE + borderPixel;
        }
        ...
        // --- Affichage des avatars/infos joueurs ---
        if (theme == null) {
            // main branch : avatars dynamiques et multiples joueurs
            ...
            // branche thème : 2 joueurs fixes (avatarP1, avatarP2)
        ...
        for (int y = 0; y < gridHeight; y++) {
            for (int xg = 0; xg < gridWidth; xg++) {
                ...
                // Optionnel : affichage de la grille dans le mode thème (branche)
                if (theme != null && grid.getCell(xg, y) != com.bomberman.model.Grid.CellType.INDESTRUCTIBLE
                        && grid.getCell(xg, y) != com.bomberman.model.Grid.CellType.DESTRUCTIBLE) {
    ...
    // Pour l'affichage block avatar dynamique (main branch)
